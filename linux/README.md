...
- Processos, árvore, categorias, origem, encerramento e proteção da sessão. O sampler lê `/proc` direto: threads não entram na lista, descritores de `/proc/*/stat` não ficam abertos, CPU é % da máquina com média de 1 s mesmo com afinidade restrita. RSS, USS e PSS têm significados distintos; USS/PSS vêm de `smaps_rollup` com teto por ciclo e cache de 5 s. Sem essa leitura, o privado usa `RSS − shared`. Falta de permissão em PSS aparece como `—` e agregações parciais como `≥`. Virtual por processo não é commit; o commit global usa Committed_AS/CommitLimit. Descritores de arquivo na ficha, com o mesmo orçamento/cache.
